<template>
  <div class="h-full">
    <section id="text">
      <div class="flex text-base-content flex-col items-center gap-40 justify-center">
        <article
          class="prose prose-h1:text-6xl md:prose-2xl prose-headings:tracking-widest text-center space-y-36 font-bold"
        >
          <h2 data-aos-duration="1000" class="tracking-widest" data-aos="fade-down">
            ถึงเวลาซักผ้าที่กองไว้ตั้งนานแล้ว
          </h2>
          <h1
            class="tracking-widest"
            data-aos-duration="1000"
            data-aos-anchor-placement="bottom-bottom"
            data-aos="fade-down"
          >
            แต่...
          </h1>
        </article>

        <div id="thinking_zone" class="my-28 overflow-hidden">
          <div id="chats-on-left-side" class="relative">
            <div class="grid grid-flow-row gap-y-12 lg:grid-flow-col place-items-center">
              <div class="flex flex-col gap-12">
                <div
                  data-aos-anchor-placement="top-center"
                  data-aos="fade-up-left"
                  class="chat chat-end"
                >
                  <div class="chat-bubble text-base sm:text-2xl chat-bubble-primary">
                    อยู่หอพักแต่ไม่มีเครื่องซักผ้าทำไงดี 🤔
                  </div>
                </div>
                <div
                  data-aos-anchor-placement="top-center"
                  data-aos="fade-down-left"
                  class="chat mr-0 md:-mr-12 chat-end"
                >
                  <div class="chat-bubble text-base sm:text-2xl chat-bubble-accent text-white">
                    อยากได้เครื่องซักผ้าที่ซักและตากผ้าอัตโนมัติได้จังเลย 🤩
                  </div>
                </div>
              </div>
              <iframe
                data-aos-anchor-placement="top-center"
                data-aos="zoom-in"
                id="thinking_animation"
                class="w-96"
                src="https://lottie.host/embed/9fe79fa3-98f9-4484-b8e9-249513c40b7d/0apgtsaVwL.json"
              ></iframe>
              <div id="chats-on-right-side" class="relative">
                <div class="flex flex-col gap-12">
                  <div
                    data-aos-anchor-placement="top-center"
                    data-aos="fade-up-right"
                    class="chat chat-start ml-0 md:-ml-12"
                  >
                    <div class="chat-bubble text-base sm:text-xl">
                      ขี้เกียจซักมือจัง อยากได้เครื่องซักผ้า แต่มันแพง 😭
                    </div>
                  </div>
                  <div
                    data-aos-anchor-placement="top-center"
                    data-aos="fade-down-right"
                    class="chat chat-start"
                  >
                    <div class="chat-bubble text-base sm:text-3xl chat-bubble-secondary">
                      ไม่อยากซักผ้าเอง เหนื่อย เสียเวลา 😫
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <article
          class="prose prose-h1:text-6xl md:prose-2xl prose-headings:tracking-widest text-center space-y-44 font-bold"
        >
          <h2 data-aos-duration="1000" data-aos="fade-down">ดูยุ่งยากใช่มั้ยหล่า</h2>
          <h1
            data-aos-duration="1000"
            class="gradient-heading bg-gradient-to-r from-primary to-secondary py-4"
            data-aos="fade-down"
          >
            งั้นให้เราช่วยคุณสิ
          </h1>
          <h2 data-aos-duration="1000" class="divider" data-aos="fade-down">
            ง่าย ๆ แค่ทำขั้นตอนตามนี้
          </h2>
        </article>
      </div>
    </section>
  </div>
</template>
